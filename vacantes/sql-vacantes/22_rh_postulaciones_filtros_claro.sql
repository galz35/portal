...
CREATE OR ALTER PROCEDURE dbo.spPost_RH_ListarTodas
AS
BEGIN
    SET NOCOUNT ON;
    ...
    FROM (
        SELECT
            ...
            CAST(NULL AS nvarchar(100)) AS DepartamentoResidencia,
            CAST(NULL AS nvarchar(100)) AS MunicipioResidencia,
            CAST(NULL AS nvarchar(80)) AS CategoriaInteres,
            CAST(NULL AS nvarchar(40)) AS ModalidadPreferida,
            CAST(NULL AS nvarchar(80)) AS NivelAcademico,
            CAST(NULL AS bit) AS DisponibilidadViajar,
            CAST(NULL AS bit) AS DisponibilidadHorarioRotativo,
            CAST(NULL AS bit) AS TieneLicenciaConducir,
            CAST(NULL AS nvarchar(40)) AS TipoLicencia,
            CAST(NULL AS bit) AS TieneVehiculoPropio
