...
async function run() {
    try {
        ...
        sql += `EXEC sp_droprolemember 'db_datareader', 'candida';\n`;
        sql += `EXEC sp_droprolemember 'db_datawriter', 'candida';\nGO\n\n`;
        ...
        fs.writeFileSync('candida_access.sql', sql);
        console.log("SQL generated to candida_access.sql");
        process.exit(0);
    } catch (e) {
        console.error(e);
        process.exit(1);
