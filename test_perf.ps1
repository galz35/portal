...
$url = "http://localhost:8082/api/auth/session-state"
$counts = 5
$totalTime = 0
...
for ($i = 1; $i -le $counts; $i++) {
    ...
    try {
        ...
        # Ignorar errores 401/404, nos interesa el tiempo de respuesta
    }
    $sw.Stop()
...
if ($avg -lt 100) {
    Write-Host "¡EXCELENTE! El rendimiento es óptimo (sub-100ms)." -ForegroundColor Green
} elseif ($avg -lt 500) {
    Write-Host "BUENO. El rendimiento es aceptable." -ForegroundColor Yellow
} else {
    Write-Host "ADVERTENCIA. Sigue habiendo latencia significativa." -ForegroundColor Red
}
